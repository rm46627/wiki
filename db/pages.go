...
func PageByURL(url string) (*Page, error) {
	...
	if err := row.Scan(&p.ID, &p.URL, &p.Body, &p.Title); err != nil {
...
// InsertComment make insert query to store data of comment in db
func InsertComment(c *Comment) (int64, error) {
	result, err := Database.Exec("INSERT INTO comments (author, content, pageId) VALUES (?, ?, ?)", c.Author, c.Content, c.PageID)
	if err != nil {
		return -1, fmt.Errorf("saving comment %s to db: %v", c.Content, err)
	}
	id, err := result.LastInsertId()
	if err != nil {
		return -1, fmt.Errorf("getting id generated by the database: %v", err)
